...
function affine.analyze_index_noninterference_self(loop_index, arg, field_name)
  ...
  elseif arg_index:is(ast.typed.expr.Binary) then
    ...
    elseif arg_index.op == "*" then
      if affine.is_constant_expr(arg_index.lhs) then
        ...
        return (coeff >= 1 or coeff <= -1)
               and affine.analyze_index_noninterference_self(loop_index, arg_index.rhs, field_name)
      elseif affine.is_constant_expr(arg_index.rhs) then
        local coeff = affine.convert_constant_expr(arg_index.rhs)
        return -1 <= coeff and coeff <= 1
               and affine.analyze_index_noninterference_self(loop_index, arg_index.lhs, field_name)
      end
    elseif arg_index.op == "/" then
  ...
  elseif arg_index:is(ast.typed.expr.Ctor) then
    -- DELETE ME
    print("affine noninterself: Ctor")
    ...
    if loop_index_type.fields then
      ...
      for _, loop_index_field in ipairs(loop_index_type.fields) do
        -- DELETE ME
        print("affine Ctor: searching for field "..loop_index_field)
        local field_present = false
        for _, ctor_field in ipairs(arg_index.fields) do
          field_present = field_present
                          or affine.analyze_index_noninterference_self(loop_index, ctor_field.value, loop_index_field)
        end
        if not field_present then
          -- DELETE ME
          print("affine Ctor: field "..loop_index_field.." not found")
          return false
        end
      end

      -- DELETE ME
      print("affine Ctor: all fields found")
      return true
    end

  elseif arg_index:is(ast.typed.expr.FieldAccess) then
    local id = arg_index.value
    return loop_index == id.value
           and field_name == arg_index.field_name
  end
